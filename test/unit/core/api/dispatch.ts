...
@suite('Unit::Core::Dispatch')
class Test {
  @test
  async 'simple dispatch returns text response'() {
    const exampleRequest = { path: '/greet' }
    const handler = require(join(__dirname, 'example', 'Greet.ts')).default
  ...
  async 'dispatching real lambda calls works'() {
    ...
    const api = require(join(__dirname, 'example', 'Greet.ts')).handler()
  ...
  async 'an api with cache sets cache-control to max-age={seconds}'() {
    ...
    const apiModule = require(join(__dirname, 'example', 'GreetCached.ts'))
